...
async function main() {
  ...
  printTest('重启后抽检任务仍存在', detailRes.statusCode === 200,
    `实际: ${detailRes.statusCode}`);

  if (detailRes.statusCode === 200) {
    const inspection = detailRes.body.inspection;
    printTest('重启后状态一致', inspection.status === finalStatus,
      `预期: ${finalStatus}, 实际: ${inspection.status}`);
    printTest('重启后版本号一致', inspection.version === finalVersion,
      `预期: ${finalVersion}, 实际: ${inspection.version}`);
    printTest('审计日志数量一致', detailRes.body.auditLogs.length === auditLogCount,
      `预期: ${auditLogCount}, 实际: ${detailRes.body.auditLogs.length}`);
    printTest('抽检项目数量正确', inspection.inspectionItems.length === 3);
    printTest('检测结果未丢失',
      inspection.inspectionItems.every(item => item.result !== ''));
    printTest('创建人信息完整', inspection.createdBy && inspection.createdByName);
    printTest('提交人信息完整', inspection.submittedBy && inspection.submittedByName);
    printTest('确认人信息完整', inspection.approvedBy && inspection.approvedByName);
  }
  ...
  printTest('重启后审计日志可访问', auditRes.statusCode === 200);

  if (auditRes.statusCode === 200) {
    const auditLogs = auditRes.body.auditLogs;
    printTest('审计日志包含创建记录', auditLogs.some(l => l.action === 'inspection_create'));
    printTest('审计日志包含提交记录', auditLogs.some(l => l.action === 'inspection_submit'));
    printTest('审计日志包含退回记录', auditLogs.some(l => l.action === 'inspection_return'));
    printTest('审计日志包含确认记录', auditLogs.some(l => l.action === 'inspection_approve'));
    printTest('每条审计日志都有时间戳',
      auditLogs.every(l => l.timestamp));
    printTest('每条审计日志都有操作者角色',
  ...
  printTest('第二个抽检任务重启后仍存在', detail2Res.statusCode === 200);

  if (detail2Res.statusCode === 200) {
    const inspection2 = detail2Res.body.inspection;
    printTest('第二个任务状态为 returned', inspection2.status === 'returned');
    printTest('第二个任务版本号为 3', inspection2.version === 3,
      `实际版本: ${inspection2.version}`);
    printTest('第二个任务退回原因存在', inspection2.returnReason !== '');
  }
  ...
  printTest('重启后列表查询正常', listRes.statusCode === 200);
  printTest('列表至少包含2条记录', listRes.body.inspections.length >= 2);

  const batchListRes = await request({
    ...baseOptions, path: `/api/inspections/batch/${verifyData.batchNo}`, method: 'GET',
    headers: headers('receiver01')
  });
  printTest('重启后按批次查询正常', batchListRes.statusCode === 200);
  printTest('按批次查询结果数量正确', batchListRes.body.inspections.length === 1);
  printTest('查询结果批次号匹配', batchListRes.body.inspections[0].batchNo === verifyData.batchNo);
  ...
  printTest('重启后状态过滤正常', approvedList.statusCode === 200);
  printTest('过滤结果中包含测试任务',
  ...
  printTest('重启后 JSON 导出正常', jsonExport.statusCode === 200);
  printTest('JSON 导出包含 inspections 数组', Array.isArray(jsonExport.body.inspections));
  ...
  printTest('重启后 CSV 导出正常', csvExport.statusCode === 200);
  printTest('CSV 导出内容为字符串', typeof csvExport.body === 'string');

  const detailJsonExport = await request({
    ...baseOptions, path: `/api/inspections/export/${inspectionId}?format=json`, method: 'GET',
    headers: headers('quality01')
  });
  printTest('重启后详情 JSON 导出正常', detailJsonExport.statusCode === 200);
  printTest('详情导出包含审计日志', Array.isArray(detailJsonExport.body.auditLogs));

  console.log('\n--- 验证组 7: 版本号和状态流转验证 ---\n');

  if (detailRes.statusCode === 200) {
    const inspection = detailRes.body.inspection;
    printTest('版本号为整数且大于0', Number.isInteger(inspection.version) && inspection.version > 0);
    printTest('状态为有效值',
      ['pending', 'submitted', 'approved', 'returned'].includes(inspection.status));
    printTest('updatedAt 时间戳存在', inspection.updatedAt && !isNaN(new Date(inspection.updatedAt).getTime()));
    printTest('createdAt 时间戳存在', inspection.createdAt && !isNaN(new Date(inspection.createdAt).getTime()));
  }
  ...
  printTest('重启后收货员仍不能创建任务', receiverCreate.statusCode === 400,
  ...
  printTest('重启后药师仍不能确认任务', pharmacistApprove.statusCode === 400,
    `实际: ${pharmacistApprove.statusCode}`);

  const allPass = failCount === 0;
  console.log('\n' + (allPass ? '✓ 所有跨重启验证通过！' : '✗ 部分验证失败，请检查。'));
  console.log(`\n总计: 通过 ${passCount} 项, 失败 ${failCount} 项`);
